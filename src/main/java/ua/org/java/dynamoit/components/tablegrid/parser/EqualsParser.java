...
public class EqualsParser<T extends Filter<T>> extends BaseValueToFilterParser<T> {

    private static final Pattern PATTERN = Pattern.compile("(.*)");
    private final Attributes.Type type;

    public EqualsParser(String value, Attributes.Type type, T filter) {
        super(value, filter);
        this.type = type;
    }
        ...
        return term -> {
            try {
                if (type == Attributes.Type.NUMBER) {
                    filter.eq(new BigDecimal(term));
                } else if (type == Attributes.Type.BOOLEAN) {
                    filter.eq(Boolean.valueOf(term));
                } else {
                    filter.eq(term);
                }
            } catch (Exception e) {
